...
class DeepSort(object):
    ...
    def update(self, bbox_xywh, confidences, ori_img, bbox_xywh_anti_occ = [],
               confidences_anti_occ = [], id_list = [], timestamp = 0):
        self.height, self.width = ori_img.shape[:2]
        # generate detections
        # 从原图中抠取bbox对应图片并计算得到相应的特征
        features = self._get_features(bbox_xywh, ori_img)
        bbox_xywh_all = torch.cat((bbox_xywh, bbox_xywh_anti_occ), dim = 0)
        conf_all = torch.cat((confidences, confidences_anti_occ), dim = 0)
        bbox_tlwh = self._xywh_to_tlwh(bbox_xywh_all)
        # 抗遮挡目标信息加入 bbox用预测的，feature用id最后时间捕捉的
        for index in range(len(id_list)):
            try:
                for traj in self.tracker.tracks:
                    if traj.track_id == id_list[index]:
                        features_occ = np.expand_dims(traj.features[-1], axis=0)
                        if len(features) == 0:
        ...
        for track in self.tracker.tracks:
            ...
            outputs.append([x1,y1,x2,y2,lines,track_id])
        return outputs
    ...
    def _get_features(self, bbox_xywh, ori_img):
        ...
        for box in bbox_xywh:
            ...
            im_crops.append(im)
        if im_crops:
            features = self.extractor(im_crops) # 对抠图部分提取特征 [目标数，512]
        else:
            features = np.array([])
        return features
